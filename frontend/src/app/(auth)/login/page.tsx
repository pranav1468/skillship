...
export default function LoginPage() {
  ...
  useEffect(() => {
    document.title = "Login — Skillship";
  }, []);
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/auth/login", {
        ...
        body: JSON.stringify({ email: userId.trim(), password, role }),
      });
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[var(--muted)]/40 px-4 py-10">
      ...
      <div className="pointer-events-none absolute inset-0 bg-grid-pattern opacity-30 [mask-image:radial-gradient(ellipse_at_center,rgba(255,255,255,0.85),transparent_70%)]" aria-hidden="true" />
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-3.5" noValidate>
            ...
            <div className="relative" ref={roleRef}>
              <button type="button" onClick={() => setShowRoleMenu((v) => !v)} className="group flex h-[52px] w-full items-center gap-3 rounded-2xl border border-[var(--border)] bg-white px-4 text-left shadow-sm transition-colors hover:border-primary/30 focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10">
                ...
                <span className={`flex-1 text-sm ${selectedRoleLabel ? "text-[var(--foreground)]" : "text-[var(--muted-foreground)]"}`}>{selectedRoleLabel ?? "Select Role"}</span>
            ...
            <div className="relative">
              ...
              <input type="email" value={userId} onChange={(e) => setUserId(e.target.value)} placeholder="Email address" autoComplete="email" className="h-[52px] w-full rounded-2xl border border-[var(--border)] bg-white pl-14 pr-4 text-sm text-[var(--foreground)] shadow-sm outline-none transition-colors placeholder:text-[var(--muted-foreground)] focus:border-primary focus:ring-4 focus:ring-primary/10" />
            </div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" autoComplete="current-password" className="h-[52px] w-full rounded-2xl border border-[var(--border)] bg-white pl-14 pr-12 text-sm text-[var(--foreground)] shadow-sm outline-none transition-colors placeholder:text-[var(--muted-foreground)] focus:border-primary focus:ring-4 focus:ring-primary/10" />
            ...
            <button type="submit" disabled={isSubmitting} aria-busy={isSubmitting} className="group relative mt-2 flex h-[52px] w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-gradient-to-r from-primary to-accent text-sm font-semibold text-white shadow-[0_12px_30px_-12px_rgba(5,150,105,0.6)] transition-all hover:-translate-y-0.5 hover:shadow-[0_16px_40px_-12px_rgba(5,150,105,0.7)] focus:outline-none focus:ring-4 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-70 disabled:hover:translate-y-0 disabled:hover:shadow-[0_12px_30px_-12px_rgba(5,150,105,0.6)]">
              {isSubmitting ? (
                <>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" className="animate-spin" aria-hidden="true"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>
                  Signing in…
                </>
              ) : (
                <>
